...
class MLOCommand(Command):
	...
	def exce(self):
		...
		if self.name.upper() == 'CREATE':
			...
			data = db.filter_recoreds(self.type_name, self.filed, self.value, self.operator)
		db.commit();
		return data
...
class HaloSoftware:
	...
	MLO_CMD = ['CREATE RECORD', 'DELETE RECORD', 'UPDATE RECORD', 'SEARCH RECORD', 'LIST RECORD', 'FILTER RECORD']
	...
	def exce(self, commands):
		with open(f'{self.output_file}', 'w') as file:
			for cmd in self.commands:
				if isinstance(cmd, AuthCommand):
				...
				elif (isinstance(cmd, DLOCommand) or isinstance(cmd, MLOCommand)) and self.user is None:
					...
					if cmd.has_output:
						if isinstance(cmd, DLOCommand) and cmd.name.upper() == 'LIST':
							...
							for key in opt.keys():
								file.write(f'{key}\n')
						elif isinstance(cmd, MLOCommand) and cmd.name.upper() == 'SEARCH':
							file.write(f'E226-S187, ')
							for value in opt.values():
								file.write(f'{value}, ')
							file.write('\n')
						elif isinstance(cmd, MLOCommand) and (cmd.name.upper() == 'LIST' or cmd.name.upper() == 'FILTER'):
							for key, values in opt.items():
								...
								for value in values.values():
									file.write(f'{value}, ')
								file.write('\n')
					logger.info(f"{self.user.username},{int(time.time())},{cmd._command},SUCCESS")
